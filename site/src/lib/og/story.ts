...
function buildStorySvg(assets: StoryAssets) {
  ...
  const leftFootCardX = 108;
  const rightFootCardX = 586;
...
<svg width="${STAGE.width}" height="${STAGE.height}" viewBox="0 0 ${STAGE.width} ${STAGE.height}" fill="none" xmlns="http://www.w3.org/2000/svg">
  <defs>
    ...
    <linearGradient id="glassHeroFill" x1="${HERO.x}" y1="${HERO.y}" x2="${HERO.x + HERO.width}" y2="${HERO.y + HERO.height}" gradientUnits="userSpaceOnUse">
      <stop offset="0%" stop-color="${rgba(assets.textMain, 0.08)}" />
      <stop offset="24%" stop-color="${rgba(assets.accentSoft, 0.07)}" />
      <stop offset="100%" stop-color="${rgba(assets.bgPrimary, 0.22)}" />
    </linearGradient>
    <linearGradient id="glassFeatureFill" x1="${SAFE.left}" y1="${FEATURE.y}" x2="${STAGE.width - SAFE.right}" y2="${FEATURE.y + FEATURE.height}" gradientUnits="userSpaceOnUse">
      <stop offset="0%" stop-color="${rgba(assets.textMain, 0.06)}" />
      <stop offset="32%" stop-color="${rgba(assets.accentSoft, 0.05)}" />
      <stop offset="100%" stop-color="${rgba(assets.bgPrimary, 0.16)}" />
    </linearGradient>
    <linearGradient id="glassFootFill" x1="${FOOT.x}" y1="${FOOT.y}" x2="${FOOT.x + FOOT.width}" y2="${FOOT.y + FOOT.height}" gradientUnits="userSpaceOnUse">
      <stop offset="0%" stop-color="${rgba(assets.textMain, 0.05)}" />
      <stop offset="40%" stop-color="${rgba(assets.accentSoft, 0.04)}" />
      <stop offset="100%" stop-color="${rgba(assets.bgPrimary, 0.16)}" />
    </linearGradient>
    <linearGradient id="glassSheen" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0%" stop-color="${rgba(assets.textMain, 0.16)}" />
      <stop offset="28%" stop-color="${rgba(assets.textMain, 0.07)}" />
